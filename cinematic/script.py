...
if __name__ == "__main__":
    import json
    total = sum(s["secs"] for _, s in shots())
    words = sum(len(t.split()) for b in BEATS for _, t in b["say"])
    print(json.dumps({"beats": len(BEATS), "shots": sum(1 for _ in shots()), "picture_secs": total, "words": words}))
